...
class AnalogInClockOut(QThread, BaseAnalogInput):
    ...
    def _configure_ai_task(self):
        try:
            ...
            self._ai_task.triggers.start_trigger.cfg_dig_edge_start_trig("/dev4/PFI0", trigger_edge=Edge.RISING)
            self._ai_task.timing.cfg_samp_clk_timing(rate=self._samplerate, samps_per_chan=self._N_samples, sample_mode=AcquisitionType.FINITE)
    ...
    def _configure_trigger_task(self):
        try:
            ...
            self._trigger_out_task.timing.cfg_implicit_timing(sample_mode=AcquisitionType.CONTINUOUS)   
        except Exception as err:
            logging.getLogger().debug(err)
            raise
